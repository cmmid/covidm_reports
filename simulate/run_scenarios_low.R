...
yu <- covidm_fit_yu[sample(.N, run_options_low[,max(index)], replace = TRUE)]
...
if (!is.na(ulim)) {
  ...
  run_options_low <- run_options_low[1:min(ulim, .N)]
  yu <- yu[1:min(ulim, .N)]
}
...
for(i in 1:nrow(run_options_low)){
  ...
  target_R0 <- run_options_low[i, r0]
  params$pop <- lapply(
    params$pop,
    function(x){
  ...
  sim <- cm_simulate(
    params, 1,
    model_seed = run_options_low[i, model_seed]
  )$dynamics
