...
plot_all_parameters <- function(){
  r0_plot <- plot_r0_distribution() + labs(subtitle="A - Basic reproductive rate") + theme(aspect.ratio=1.0)
  case_fatality_plot <- plot_case_fatality_ratio()+ labs(subtitle="B - Case fatality ratio")+ theme(aspect.ratio=1.0)
  serial_interval_plot <- plot_serial_interval_ratio()+ labs(subtitle="C - Serial interval")+ theme(aspect.ratio=1.0)
  yis_plot <- plot_yis()+ labs(subtitle="D - Age-specific clinical fraction\n")+ theme(aspect.ratio=1.0)
  fs_plot <- plot_fs()+ labs(subtitle="E - Relative infectiousness in\n     subclinical cases")+ theme(aspect.ratio=1.0)
  delay_symp <-  plot_delay_symptom()+ labs(subtitle="F - Delay from symptom\n     onset to hospitilisation")+ theme(aspect.ratio=1.0)
