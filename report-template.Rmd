...
rngprint <- function(r) {
  ifelse(round(r[1])==round(r[2]),as.character(round(r[1])), sprintf("%i-%i",round(r[1]), round(r[2])))
}
...
landscape(collapse_rows(kable(unmitacctable1, caption="Cumulative counts for various outcomes in the unmitigated scenario, by age group; counts are evaluated at 1, 2, 3 months, then every 3 months.")))
```

```{r uncumulative2}
landscape(collapse_rows(kable(unmitacctable2, caption="Continuation of cumulative counts for the unmitigated scenario.")))
```
...
The panels in Fig. \@ref(fig:tsintervention) compare epidemics without intervention with 5 different potential interventions. The quantiles for peak timing and values for those outcomes is summarised in Table \@ref(tab:intpeakstable); Tables \@ref(tab:intcumulative1) and \@ref(tab:intcumulative2) covers cumulative reductions due to the interventions at particular milestones.

```{r tsintervention, fig.height=4, fig.cap="Epidemics with interventions for all ages. Time series and peak sizes for incidence of symptomatic cases, incidence of deaths, general ward hospital occupancy, critical care hospital occupancy, and total hospital occupancy (general ward plus critical care)."}
pks <- alls.dt[age=="all",{
  res <- apply(.SD, 2, max)
  names(res) <- colnames(.SD)
  as.list(res)
...
landscape(collapse_rows(kable(intacctable1, caption="Cumulative reductions for various outcomes by intervention scenario; reductions are evaluated at 1, 2, 3 months, then every 3 months.")))
```

```{r intcumulative2}
landscape(collapse_rows(kable(intacctable2, caption="Continuation of cumulative reductions for various intervention scenarios.")))
```
