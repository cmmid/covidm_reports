...
summary_tsplotter <- function(subdt, title, peak.title = "Peak Number", timing.title = "Peak Timing", pop = poppyra) {
  ...
  cumulative_median_values <- refdt %>%
    group_by(compartment, run) %>%
    arrange(date) %>%
  ...
  p <- refdt %>%
    ...
    geom_hline(
      ...
      aes(yintercept = mean_peak, colour=compartment), linetype = "dotted"
    ) +
    ...
    scale_fill_manual("", values=brewer.pal(n=4, name="Set2"),
                          breaks=compartments, labels = compartment_names) +
    scale_y_continuous("Number of cases", labels = scales::label_number_auto()) +
  ...
  p2 <- plot_grid(
    ...
    ptiming + no_legend(),
    nrow = 2, ncol = 1,
    labels = c("A) Case numbers", "B) Peak timings"),
    hjust = 0,
    label_size = 8,
    rel_heights = c(3, 1),
    align = "hv",
...
tsplotter <- function(subdt, title, peak.title = "Peak Number", timing.title = "Peak Timing", pop = poppyra) {
  ...
  ptiming <- ggplot(timing.dt) +
    aes(y = age, yend = age, color = age) +
  ...
  ppeak <- ggplot(size.dt) +
    aes(x = age, xend = age, color = age) +
...
tsCplotter <- function(subdt, title, peak.title, timing.title) {
...
summary_tsCplotter <- function(subdt, title, peak.title, timing.title) {
