...
summary_tsplotter <- function(subdt, title, peak.title = "Peak Number", timing.title = "Peak Timing", pop = poppyra) {
  ...
  p <- refdt %>%
    ...
    geom_ribbon(aes(x=date, 
                    ...
                    alpha=0.3)+
    #geom_line(aes(x=date, y=mean_value))+
...
tsplotter <- function(subdt, title, peak.title = "Peak Number", timing.title = "Peak Timing", pop = poppyra) {
  ...
  ppeak <- ggplot(size.dt) + aes(x=age, xend=age, color=age) +
    ...
    scale_y_continuous(peak.title, sec.axis = dup_axis(), labels=scales::number_format()) +
    #dtscale +
    scale_color_pop(guide = "none") +
    thm_right
...
tsCplotter <- function(subdt, title, peak.title, timing.title) tsplotter(subdt[between(date, refday, refday+ulim)][,
  .(value = sum(value)),
  keyby = .(run, date, population, compartment, age)
][,
  .(value = cumsum(value), date),
  keyby = .(run, population, compartment, age)
], title, peak.title, timing.title)

summary_tsCplotter <- function(subdt, title, peak.title, timing.title) summary_tsplotter(subdt[between(date, refday, refday+ulim)][,
  .(value = sum(value)),
  keyby = .(run, date, population, compartment, age)
][,
  .(value = cumsum(value), date),
...
summary_tsCplotter(subdyn[compartment == "death_o"], "Cumulative Deaths", "Final Number", "Last Timing")
```
```{r echo=FALSE}
...
```{r echo=FALSE, fig.height=3}
tsCplotter(subdyn[compartment == "death_o"], "Cumulative Deaths", "Final Number", "Last Timing")
```
