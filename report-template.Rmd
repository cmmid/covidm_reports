...
- We estimate predicted peak counts and timing in incident symptomatic cases, deaths and daily hospital bed need, and calculate the impact of the interventions relative to an unmitigated epidemic.
...
- We assume the outbreak is seeded with 50 initial infections, proportionally distributed according to the age demography of `r params$location`. We refer to this day 0 as the *initial introduction*.
- These reports will be updated as `r params$location`-specific details become available, *e.g.*: local parameter estimates, appropriate timing and number of initial introductions, appropriate sub-national populations, and planned or executed interventions are available.
- Modelling methods and parameters are given at the end of the report.

**All numerical values are reported to two significant figures only.**

# Unmitigated COVID-19 Epidemic Trajectory

The panels in Fig. \@ref(fig:tsunmit) show aggregate and by-age group outcomes for daily incidence of COVID-19 cases and deaths in `r params$location`, and daily demand for hospital care. The ranges for peak timing and values for those outcomes is summarised in Table \@ref(tab:unpeakstable) (by 50% and 95% forecast intervals); Table \@ref(tab:uncumulative1) covers cumulative counts at 3, 6, 9, and 12 months from initial introduction in `r params$location`.
...
rngprint <- function(r, prefix="") {
  ifelse(down(r[1])==up(r[2]),format(signif(r[1],2),big.mark = ","), sprintf("%s%s - %s",prefix,format(down(r[1]), big.mark = ","), format(up(r[2]),  big.mark = ",")))
}
...
tabltext <- function(filterdt) {
  paste0(rngprint(filterdt[,c(lo,hi)])," (",rngprint(filterdt[,c(lo.lo,hi.hi)]),")")
}
...
In the event of unmitigated epidemic, we anticipate that in `r params$location` the total number of symptomatic cases one year from initial introduction is `r bullettext(outcomesc[compartment == "cases"])` and the total number of deaths at that time is `r bullettext(outcomesc[compartment == "death_o"])`. Table \@ref(tab:sumtable) shows the size and timing of peaks for various outcomes.

```{r sumtable}
tabl <- data.table(
  `Outcome` = c(
    "Incidence of Symptomatic Cases",
    "Overall Hospitalisation Demand",
    "Non-critical Hospitalisation Demand",
    "Critical Hospitalisation Demand",
    "Incidence of Deaths"
  ),
  `50% interval on peak count (95% interval)` = c(
  ...
  `50% interval on peak timing in days (95% interval)` = c(
...
kable_styling(
  kable(tabl, caption=sprintf("Overview of estimated peak impacts based on the population in %s. Attack rates of symptomatic disease given infection, death and hospitalisation given symptomatic disease are based on global estimates of those values. Relative rates of hospitalisation severity demands are also based on global estimates. Timing is in days from initial introduction to %s."), params$location, params$location),
...
tabpeak <- alls.dt[scen_id == 1,{
  ...
  .(
    t=sprintf("%s (%s)", rngprint(c(t.hi,t.lo)), rngprint(c(t.hh, t.ll))),
    v=sprintf("%s (%s)", rngprint(c(v.lo,v.hi)), rngprint(c(v.ll,v.hh)))
  )},
  by=.(age, compartment)
]
reord(tabpeak)
...
mkkbl <- function(tbl, caption, b = TRUE) row_spec(
  collapse_rows(
  kable(
    tbl,
    caption=caption
...
mkkbl(
  tabpeak,
  caption="Peak timing and values for main outcomes in the unmitigated scenario, by age group. Timing is from the date of initial introduction."
)
```
...
order(compartment, age),.(
  `Total Outcomes`=outcome_cumulative[as.character(compartment)],
  `Age Group`=sprintf("%s %s",as.character(age),ifelse(age=="all","ages","years")),
  `3 months`=`90`,
  `6 months`=`180`,
  `9 months`=`270`
...
landscape(mkkbl(
    unmitacctable1,
    caption="Total counts for various outcomes in the unmitigated scenario, by age group; counts are evaluated at 3, 6, and 9 months from initial introduction."
))
```
...
```{r tsintervention, fig.height=6, out.width="10in", fig.width=10, fig.cap="Epidemics with interventions for all ages. Time series and peak sizes for incidence of symptomatic cases, incidence of deaths, general ward hospital occupancy, critical care hospital occupancy, and total hospital occupancy (general ward plus critical care)."}
pks <- alls.dt[age=="all",{
  res <- apply(.SD, 2, max)
  names(res) <- colnames(.SD)
  as.list(res)
...
mkkbl(tabpeak, caption="Peak timing and values for main outcomes, by intervention scenario. Timing is from the date of ongoing community spread with 50 infections.", b = FALSE)
```
...
landscape(mkkbl(
  intacctable1,
  caption="Cumulative reductions for various outcomes by intervention scenario; reductions are evaluated at 3, 6, and 9 months.",
  b = FALSE
))
```
