...
# TODO work out how this should be calculated properly.
calculate_total_deaths_date <- function(subdt){
  mean(subdt %>% 
    group_by(run, date) %>% 
    ...
    ungroup() %>% 
    group_by(date) %>% 
    summarise(median_value=round(median(cum_value))) %>% 
    filter(median_value >= 13500) %>% select(date) %>% pull())
}
...
Figure 1 shows that the median peak number of symptomatic cases across all model projections is 500 000, forecast to occur on `r cases_peaks_dates$cases_reported`. The predicted peak in hospitalised cases occurs about 15-16 days later (Figures 2 and 3), with a median of 35,000 cases in ward and 18,000 in ICU on `r peaks_dates$nonicup_p`. The peak daily number of deaths is predicted to follow about 10 days later (`r peaks_date$death_o`), with the median estimate of 750 new deaths that day (Figure 4). Figure 5 shows that the cumulative number of deaths is predicted to be 13 500 (median), occurring on ***insert date/range if data available*** (about 107 days after the first reported case). 
